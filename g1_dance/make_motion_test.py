...
规范要点:
 - 大臂基本固定(抱架-0.74, 出拳略沉到-0.60), 拳水平向前不抬
 - 出拳末端肘留~8°(-0.15)不锁死, 停0.15s让"伸直"可见
 - 出拳用 ease_out 快攻(非匀速); 非出拳手保持胸口护架
 - 腰yaw转向出拳方(左拳-0.20/右拳+0.20), 上身轻转不前倾
腿全程稳定站立。输出 test.csv: [root_xyz, qx,qy,qz,qw, 29关节]"""
import os, numpy as np

WS = r'C:\Users\24668\Desktop\mujoco-3.10.0-windows-x86_64'
OUT = os.path.join(WS, 'g1_dance', 'test.csv')

LEG = [-0.312, 0.0, 0.0, 0.669, -0.363, 0.0]
W0P = [0.0, 0.0, 0.0]   # waist [roll, pitch] 默认0; yaw 由事件给出(单独处理)
...
# 注: 实测 G1 肘"伸直"≈+1.5(内角167°), elbow=0 只是90°直角。以前用负值/0 → 出拳时手臂一直弯着(GPT发现的根因)。
